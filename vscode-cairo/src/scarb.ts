...
export class Scarb {
  public constructor(
    ...
    /**
     * The exact Scarb binary used can vary depending on workspace configs, hence we associate workspace folder
     * reference with the Scarb instance.
     */
    public readonly workspaceFolder?: vscode.WorkspaceFolder | undefined,
  ) {}

  public languageServerExecutable(ctx: Context): lc.Executable {
    return {
      command: this.path,
      args: ["cairo-language-server"],
      options: {
        cwd: this.workingDirectory(ctx),
      },
    };
  }
  ...
  private workingDirectory(ctx: Context): string {
    return this.workspaceFolder?.uri.fsPath ?? ctx.extension.extensionPath;
  }
}
