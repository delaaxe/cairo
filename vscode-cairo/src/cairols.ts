...
/**
 * Tries to find the development version of the language server executable, assuming the workspace directory is inside
 * the Cairo repository.
 *
 * @remarks
 *
 * This function does not attempt to go further than 10 levels up the directory tree.
 */
...
async function getServerOptions(ctx: Context): Promise<lc.ServerOptions> {
  ...
  if (serverType === ServerType.Scarb) {
    serverExecutable = scarb!.languageServerExecutable(ctx);
  } else {
